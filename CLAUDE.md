...
- **VL-6** PokéMatch("닮은 포켓몬 찾기") — 앱 완성, 자산 커밋됨, 브라우저 end-to-end 검증됨(실기기 최종확인만 남음):
  - **⚠️⚠️ 최종 판정자는 LLM이다 (2026-08-09, 사용자 요청 "포켓매치가 좀 별로다")**: 아래의 z-score 랭킹 엔진은 이제 **폴백**이고, 실제로 보이는 결과는 **Groq의 `openai/gpt-oss-120b`** 가 고른다. 임베딩은 후보를 추리는 데만 쓴다.
    ...
    - **⚠️ 후보는 "큐레이션된 40종"이 기본 (2026-08-09 사용자 지시 "보여주기 좋은 40종을 선정해서 그 안에서 되도록")**: `lib/pokematch/curatedPool.ts`의 `CURATED_POOL`이 **항상 통째로** 후보에 들어가고, 임베딩이 뽑은 **와일드카드 6종**이 뒤에 붙는다(프롬프트가 "추천 목록"과 "그 외 후보"로 나눠 보여주고 와일드카드는 **최대 1마리**만 쓰라고 지시). 이유는 **이 앱의 재미가 "아는 포켓몬"에 달려 있어서** — 아무도 모르는 종이 1위로 나오면 결과가 아니라 실패다.
      - **⚠️ 고정 풀인데도 사람마다 결과가 다른 이유**: 40종 각각을 **그 사람의 임베딩으로 채점해서**(`matcher.ts`의 **`scoreSlugs`**) z를 붙여 보낸다. 풀은 **누가 이길 수 있는지**를 고정할 뿐 **누가 이기는지**는 고정하지 않는다. 이걸 없애고 순위 없는 이름 목록만 보내면 예전의 "다들 같은 결과" 문제가 그대로 돌아온다.
      - **⚠️ 각 항목의 `look`(생김새 한 줄)은 장식이 아니다**: 모델이 스프라이트를 못 보므로, 이게 없으면 종별 외모를 **기억에만 의존**해서 매칭한다. 얼굴 측정값 ↔ `look`의 대조가 판정의 실제 알맹이다. 지우지 말 것.
      - **선정 기준(편집할 때 지킬 것)**: ① 알아볼 수 있을 것(1·2세대 위주 + 진짜 유명한 후대 종) ② 얼굴이 읽힐 것 ③ 인상이 겹치지 않게 분산(날카로움/우아함/귀여움/듬직함/개성 6그룹으로 묶어놨다) ④ **모욕이 되지 않을 것** — 질퍽이·잠만보·잉어킹·슬리퍼류는 **의도적으로 뺐다**(입력이 남의 얼굴이다). `tests/pokematchCurated.test.ts`가 이 금지 목록과 실제 갤러리 존재 여부를 검사한다.
    - **파이프라인**: MobileCLIP 임베딩 → **`scoreSlugs`(큐레이션 40종 채점)** + **`buildCandidates`(와일드카드 6종, 큐레이션 제외)** → `/api/pokematch/judge` → 모델이 5마리 + **한 줄 이유**를 고름. `buildCandidates`는 `matchTopK`의 하드코딩 제외목록·NMS를 **일부러 적용 안 한다**(그 필터들은 "공식이 좋고 나쁨을 판단 못 해서" 있던 것이고 그 판단을 모델에 넘겼기 때문. 얼굴이 아예 없는 fish/bug-wings/tentacles/squiggle만 뺀다).
    - **파일**: `lib/pokematch/faceFeatures.ts`(측정), `lib/pokematch/curatedPool.ts`(40종 + `look`), `lib/pokematch/judgeProtocol.ts`(프롬프트·파싱, **순수 함수라 `node --test`로 검증됨**), `app/api/pokematch/judge/route.ts`(전송·인증·레이트리밋만), `lib/pokematch/llmJudge.ts`(클라이언트). 라우트를 얇게 유지한 이유는 LLM 출력 파싱이 빌드로는 안 잡히는 부분이라 테스트가 필요해서다.
    - **⚠️ 후보는 번호로 주고받는다 (2026-08-09 사용자 지시 "영문 나오는건 다 빼고")**: 프롬프트의 후보에 **slug(영문)를 안 쓴다** — `1. 피카츄 · 동그란 얼굴에 볼이 도톰… · z=2.1` 처럼 **번호 + 한국어 이름 + look + z**만 준다. 모델도 번호로 답한다(`{"picks":[{"n":7,"reason":"…"}]}`). 이유 두 가지: ① 번호가 slug보다 싸다(후보 46개 × ~7토큰) ② **없는 번호는 배열에서 안 풀리므로 환각 자체가 불가능**하다. 번호는 `buildUserPrompt`에 넘긴 **배열 순서 기준 1-based**이고 섹션을 나눠 출력해도 전역 번호를 유지한다 — 라우트에서 `candidates` 배열을 **정렬하거나 재구성하면 매핑이 어긋난다**. (`normalizePicks`는 구형 `{slug}` 형태와 맨 숫자 배열도 관용적으로 받는다.)
    ...
    - **⚠️ 토큰 예산 (실측, o200k 토크나이저)**: 시스템 프롬프트 545 + 얼굴 측정문 332 + 후보 46종(추천 40 + 와일드 6) = **입력 2,455** + 출력 114 + 추론 250~700 → **회당 약 2,800~3,300토큰**. Groq **무료 티어**는 30 RPM / 1K RPD / **8K TPM** / **200K TPD**라 실질 한도는 **하루 61~70회**(요청 수 한도는 절대 못 채운다, 걸리는 건 TPD). 도감 전체 872종을 보내면 회당 35,279토큰 = **1회가 분당 한도의 4.4배라 호출 자체가 불가능**하다 — 후보를 늘리려는 생각이 들면 이 숫자를 먼저 볼 것(실질 상한 약 180종). 한도 초과 시 429 → 로컬 폴백이라 앱은 안 죽는다. 재측정 스크립트는 세션 스크래치패드에 있었고, `judgeProtocol.ts`를 import해서 실제 프롬프트를 세는 방식이다.
...
- **PokéMatch LLM 판정 실사용 확인 (2026-08-09)**: Vercel에 `GROQ_API_KEY`를 넣은 뒤 ① 결과 5마리가 사람마다 갈리는지 ② 이유 문장이 실제 얼굴 특징을 인용하는지(뭉뚱그린 소리만 하면 `faceFeatures.ts`에 측정을 더 넣을 것) ③ 판정 대기시간이 체감상 견딜 만한지 ④ "AI 판정을 불러오지 못해…" 안내가 뜨지는 않는지 ⑤ 큐레이션 40종이 실제로 만족스러운지(빠진 유명종/빼야 할 종). `?debug`로 보낸 측정문·후보·엔진을 그대로 볼 수 있다.
