...
- **VL-7** VrmMotion("VRM Capture", `/vrmmotion`) — 실시간 3D 모션캡쳐 앱, 2026-08-02~06 구현:
  ...
  - **얼굴+손 모드 (2026-08-06, 기본값)**: 앉아서 쓰는 앱에서 전신은 거의 안 쓰이므로 `TrackingMode = "upper" | "full"` 을 두고 **`upper`가 기본**(`app/vrmmotion/page.tsx`의 "트래킹 범위" 토글, 3D 카메라도 `setFraming`으로 흉상 프레이밍). `upper`는 다리 본을 아예 구동하지 않는다(무릎 visibility 게이팅보다 강한 보장).
    ...
    - **손**: `lib/handLandmarker.ts`(`hand_landmarker.task`, numHands 2, GPU→CPU 폴백) + `Kalidokit.Hand.solve` → `lib/vrm/boneRig.ts`의 `applyHandRig`. 손은 가장 비싼 단계라 **2프레임마다 1번**만 추론하고(`HAND_EVERY_N_FRAMES`) 결과를 유지한다(안 그러면 한 프레임씩 rest 포즈로 튄다).
    - **⚠️ 축 규약이 pose와 hand에서 다르다 (2026-08-06 2차 수정, 소스 확인)**: `kalidokit.es.js`의 `rigFingers`를 직접 읽어 확인 — **손가락은 Z 축 하나로만 구동**되고(`{x:0,y:0,z:angleBetween3DCoords(...)}`), Kalidokit이 이미 **좌우 보정 + 해부학적 반쪽 범위로 clamp** 한다(오른손 `[-π,0]`, 왼손 `[0,π]`). 그래서 손가락에는 **flipZ를 걸면 안 된다** — 거울이 되는 게 아니라 **손가락이 전부 뒤로 꺾인다**(실측: 왼손 커브가 +가 아니라 **−1.207rad ≈ 69° 역굽힘**). pose 본은 flipZ 유지, **손가락만 예외**.
    ...
    - **검증**: `tests/handTracking.test.ts` — 엄지 밀림 매핑, 손가락 이름(Mixamo/VRoid/Blender), **손가락 커브의 부호**(왼손 +Z / 오른손 −Z), 손목 roll이 pose에서 오는지, world visibility 병합, 그리고 blendshape 거울 매핑.
      - **⚠️ 테스트 교훈: 거리는 방향이 아니다.** 처음엔 "주먹 쥐면 손끝~손목 거리가 짧아진다"로 검증했는데, **뒤로 꺾여도 거리는 똑같이 짧아져서** 부호가 반대인 코드가 그대로 통과했다(그게 실기기 역굽힘 버그로 나왔다). 지금은 정규화 본의 **로컬 Z 부호**를 직접 본다 — flipZ를 되돌리면 실제로 실패하는 것까지 확인함.
