...
- **VL-6** PokéMatch("닮은 포켓몬 찾기") — 앱 완성, 자산 커밋됨, 브라우저 end-to-end 검증됨(실기기 최종확인만 남음):
  - **⚠️⚠️ 최종 판정자는 비전 LLM이다 (2026-08-09, 사용자 요청 "포켓매치가 좀 별로다" → "이미지로 분석하는 게 아니지?")**: 아래의 z-score 랭킹 엔진은 이제 **폴백**이고, 실제로 보이는 결과는 **Groq의 `qwen/qwen3.6-27b`(비전)** 가 **잘라낸 얼굴 사진을 직접 보고** 고른다.
    ...
    - **⚠️ 후보 풀은 규칙으로 계산한다 (291종, 사용자 지시 "적어도 200종")**: `lib/pokematch/curatedPool.ts`의 `buildCuratedPool(pokedex, gallery.species)` = **1세대 전체 + `EXTRA_FAMOUS`(후대 유명종) − `BANNED_SLUGS` − 갤러리에 없는 것**, **dex 순 정렬**. 200개를 손으로 나열하지 않고 규칙으로 만든 이유는 편집이 "목록 감사"가 아니라 "규칙 한 줄 수정"이 되게 하려는 것.
      ...
      - **⚠️⚠️ 후보는 매 스캔 섞어서 보낸다 (2026-08-15, 실측)** — `buildCuratedPool`은 dex 순으로 주지만 `usePokematchScan`의 `shuffled()`가 섞은 뒤 판정에 보낸다. **위 경고와 모순되지 않는다**: 금지 대상은 "한 요청 안에서 순서가 어긋나는 것"이고, 요청마다 순서가 달라지는 건 문제없다.
        - **근거**: dex 순으로 보냈더니 한 판정이 291종 중 **23·36·51·53·57번**을 골랐다 — 다섯 개가 전부 앞 60번 안. 무작위라면 약 **0.04%**. dex 순은 **앞쪽 = 1세대 = 유명하고 둥근 디자인**이라 "목록의 앞"과 "무난한 답"이 같은 집합이었고, 그래서 편향이 결과만 봐서는 안 보였다.
        - **섞은 뒤**: 같은 얼굴에서 **211·225·162·218·267**번이 나왔고, 뽑힌 종도 둥근 덩어리(고라파덕·푸린·발챙이)에서 날카로운 인상(리오르·조로아·조로아크)으로 바뀌었다 — 측정값의 "올라간 눈매"와 훨씬 맞는다.
        - **되돌리지 말 것.** dex 순으로 돌리면 이 편향이 조용히 되살아난다. `?debug`의 `후보#번호`가 앞쪽에 몰리는지로 감시할 수 있다.
        - **⚠️ 그냥 무작위로 섞으면 안 된다 — 얼굴에서 시드를 뽑는다** (`lib/pokematch/candidateOrder.ts`). 매번 새로 섞었더니 **같은 얼굴 두 번 스캔에 겹치는 종이 0개**가 됐다("안경·날카로운 눈매"에 맞는 종이 수십 개라, 순서가 바뀌면 그중 어디에 착지할지가 다시 굴려진다). 닮은 포켓몬은 주사위가 아니라 얼굴의 속성이어야 한다.
          - 시드는 **z 상위 10종의 slug**에서 뽑는다(정렬 후 해시). 임베딩 float은 프레임마다 흔들리지만 **상위 랭킹 집합은 같은 사람에게 안정적**이다 — 사용자의 연속 3회 스캔에서 z 값은 전부 달랐는데 상위 10종은 **매번 같은 10종**이었다. 이 실측이 설계의 근거이고 `tests/candidateOrder.test.ts`가 그 데이터를 그대로 픽스처로 쓴다.
          - 이러면 **사람 사이에는 순서가 다르고(전역 편향 없음) 한 사람 안에서는 재현된다**는 두 성질이 동시에 성립한다.
