...
- **VL-6** PokéMatch("닮은 포켓몬 찾기") — 앱 완성, 자산 커밋됨, 브라우저 end-to-end 검증됨(실기기 최종확인만 남음):
  - **⚠️⚠️ 최종 판정자는 비전 LLM이다 (2026-08-09, 사용자 요청 "포켓매치가 좀 별로다" → "이미지로 분석하는 게 아니지?")**: 아래의 z-score 랭킹 엔진은 이제 **폴백**이고, 실제로 보이는 결과는 **Groq의 `qwen/qwen3.6-27b`(비전)** 가 **잘라낸 얼굴 사진을 직접 보고** 고른다.
    ...
    - **⚠️ 토큰 예산 (실측, o200k 토크나이저)**: 시스템 431 + 유저(얼굴 측정문 332 + 후보 291종 목록) 1,896 + 이미지 448² ≈ 256 = **입력 약 2,583** + 출력 114 + 추론 300~900 → **회당 약 3,000~3,600토큰**. Groq **무료 티어**는 30 RPM / 1K RPD / **8K TPM** / **200K TPD**라 실질 한도는 **하루 55~66회**(요청 수 한도는 절대 못 채운다, 걸리는 건 TPD). 한도 초과 시 429 → 로컬 폴백이라 앱은 안 죽는다.
      - **⚠️ 291종이 46종 시절과 예산이 거의 같다**는 게 핵심이다 — 종당 비용을 46토큰(`look` 포함)에서 5.6토큰(번호+영문 이름)으로 줄여서 산 것이다. 종을 더 늘리는 것보다 **종당 설명을 되살리는 쪽이 훨씬 비싸다**. 이미지 토큰은 Qwen-VL 계열의 28px/토큰 공식으로 **추정**한 값이라 실제 청구는 실키로 확인할 것.
      - 예산이 빠듯해지면 절삭 순서: ① 얼굴 측정문(332) ② 풀 크기 ③ `reason`(114 → 14, **효과 미미하니 마지막**).
