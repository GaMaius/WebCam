...
- **VL-6** PokéMatch("닮은 포켓몬 찾기") — 앱 완성, 자산 커밋됨, 브라우저 end-to-end 검증됨(실기기 최종확인만 남음):
  - **⚠️⚠️ 최종 판정자는 LLM이다 (2026-08-09, 사용자 요청 "포켓매치가 좀 별로다")**: 아래의 z-score 랭킹 엔진은 이제 **폴백**이고, 실제로 보이는 결과는 **Groq의 `openai/gpt-oss-120b`** 가 고른다. 임베딩은 후보를 추리는 데만 쓴다.
    ...
    - **⚠️ 토큰 예산 (실측, o200k 토크나이저)**: 시스템 프롬프트 566 + 얼굴 측정문 332 + **추천 1종당 ~46** + 와일드카드 + 출력 202 + 추론 250~700 → **회당 약 3,400~3,900토큰**. Groq **무료 티어**는 30 RPM / 1K RPD / **8K TPM** / **200K TPD**라 실질 한도는 **하루 50~58회**(요청 수 한도는 절대 못 채운다, 걸리는 건 TPD). 도감 전체 872종을 보내면 회당 35,279토큰 = **1회가 분당 한도의 4.4배라 호출 자체가 불가능**하다 — 후보를 늘리려는 생각이 들면 이 숫자를 먼저 볼 것(실질 상한 약 180종). 한도 초과 시 429 → 로컬 폴백이라 앱은 안 죽는다. 재측정 스크립트는 세션 스크래치패드에 있었고, `judgeProtocol.ts`를 import해서 실제 프롬프트를 세는 방식이다.
