...
- **VL-4** 통합 결과지: 두 결과를 Canvas로 렌더링해 이미지 저장/공유(Web Share API, 다운로드 폴백)
...
- 분석(BPM/톤/얼굴형)은 온디바이스(브라우저)에서 계산되고, 원본 영상은 B2로 전송된다. **절대로 "영상이 서버로 안 간다"는 거짓 문구를 넣지 말 것** (과거에 넣었다가 강하게 정정받음).
  - 단, 2026-07-24 사용자 지시로 **랜딩/앱 화면에서 영상 전송·저장 고지 문구는 전부 제거**했고, 별도 개인정보 페이지도 두지 않기로 함. 즉 "전송된다"고 앱에 표기할 의무는 없지만, 반대로 "전송 안 된다"는 거짓 문구는 여전히 금지. README(개발자용 문서)에는 사실(전송·저장됨)을 기재해 둔 상태.
