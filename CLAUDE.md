...
- **VL-6** PokéMatch("닮은 포켓몬 찾기") — 앱 완성, 자산 커밋됨, 브라우저 end-to-end 검증됨(실기기 최종확인만 남음):
  - **⚠️⚠️ 최종 판정자는 비전 LLM이다 (2026-08-09, 사용자 요청 "포켓매치가 좀 별로다" → "이미지로 분석하는 게 아니지?")**: 아래의 z-score 랭킹 엔진은 이제 **폴백**이고, 실제로 보이는 결과는 **Groq의 `qwen/qwen3.6-27b`(비전)** 가 **잘라낸 얼굴 사진을 직접 보고** 고른다.
    ...
    - **⚠️⚠️⚠️ 판정 결과를 거르는 건 이제 `buildJudgeAllowlist`(958종)다 — 큐레이트 풀(294종)이 아니다 (2026-08-16 실측)**: 후보 목록을 프롬프트에서 뺀 뒤로 모델은 **1,000종 전체에서 자유롭게 이름을 쓴다.** 그런데 답을 294종 풀로만 받고 있었고, 실측에서 **모델이 고른 8종 중 4종이 그 이유로 삭제**됐다 — **Aipom(에이팜)·Braixen(테르나)·Emolga(에몽가)·Purrloin(쌈딱)**. 하나도 마이너하지 않고, 테르나는 **펜네킨·마폭시가 둘 다 풀에 있는데 중간진화만 빠진** 경우다. 5마리를 약속한 화면에 4장이 떴고, **살아남은 4종(델빌·판짱·조로아·리오르)이 오히려 좋은 답이었다.**
      - 풀은 목적이 둘이었는데 **하나만 남았다**: ① **인지도**("아무도 모르는 종은 결과가 아니다") → 이건 이제 **프롬프트와 모델의 몫**이다. 모델은 에몽가·에이팜을 고르지 클링클랑을 고르지 않는다. 1,000종 중 294종만 받는 건 **일어나지도 않는 나쁜 답을 막으려고 좋은 답을 지우는 것**이다. ② **금지 목록** → 이건 취향이 아니라 **보증**이라 그대로 하드 게이트다.
      - `buildJudgeAllowlist(pokedex, gallery.species)` = **갤러리에 있고 pokedex에 있고 `BANNED_SLUGS`가 아닌 전부** = 958종. `MAX_CANDIDATES`를 1200으로 올렸다(후보는 **토큰이 안 되므로** 개수는 비용이 아니다).
      - **⚠️⚠️ 그런데 넓히기만 하면 안 된다 — 큐레이트 풀은 "선호"로 남는다 (같은 날, 바로 다음 스캔에서 확인)**: 넓힌 직후 스캔에서 8개 전부 통과했는데 **결과가 더 나빠졌다** — 1등이 **크로뱃(박쥐)**, 이어서 **노고치·스컹탱크·꼬지지**. 8개 중 사람으로 읽히는 건 **조로아·앱솔·식스테일** 셋뿐이었고, **그 셋이 정확히 큐레이트 풀 멤버였다.** 즉 좁은 게이트는 인지도 필터일 뿐 아니라 **품질 필터로도 실제로 일하고 있었다.** "인지도는 프롬프트와 모델의 몫"이라고 적었던 건 **틀렸다.**
        - 그래서 `pickGuards.ts`의 **`preferFamiliar`**: 풀 멤버가 **앞으로 정렬**되고, 그룹 안에서는 **모델의 순서가 그대로 유지**된다. 풀 밖 종도 **유명한 게 5개가 안 되면 그대로 표시**된다(에몽가가 나올 수 있는 이유). **삭제가 아니라 순위다.**
        - **⚠️ 실루엣 상한(`MAX_PER_SHAPE`)도 여기에 양보한다**: 조로아·앱솔·식스테일이 **셋 다 `quadruped`** 라서 상한이 식스테일을 밀어내고 그 자리를 **크로뱃**에게 줬다. 그래서 **상한에 걸린 친숙한 픽이 상한에 안 걸린 낯선 픽보다 우선**한다. 같은 실루엣 3개는 *단조로운 5개*고, 좋은 것 2개 + 박쥐·뱀·스컹크는 *틀린 5개*다. 단조로운 쪽이 덜 나쁘다.
        - **⚠️⚠️ 표시 5마리는 4단계 우선순위로 조립한다 (`usePokematchScan`, 사용자 "크로뱃이나 야도킹 들어오는건 보기 싫다")**: 크로뱃·야도킹이 4·5번에 남은 건 **거기 채울 게 그것뿐**이어서지 좋다고 판단해서가 아니었다. 그래서 순서를 이렇게 잡았다 — ① **모델 픽 중 풀 멤버**(모델 순서 그대로) → ② **로컬 랭킹을 풀 멤버로 제한한 것** → ③ 모델의 나머지 픽(아무리 이상해도) → ④ 로컬 랭킹 전체. **②가 핵심이다**: 박쥐를 앉힐 자리에 점수가 실제로 있는 친숙한 종이 거의 항상 존재한다. ③이 남아 있으니 **삭제는 아니고**, 두 대안에 지기만 하면 된다.
        - **⚠️⚠️⚠️ `REQUEST_PICK_COUNT`는 8이다 — 절대 올리지 말 것 (2026-08-16, 12로 올렸다가 되돌림)**: "친숙도 정렬이 고를 후보를 늘리자"는 이유로 12로 올렸는데 **판정 자체가 무너졌다.** 4회 실측:
          | 요청 | 결과 |
          |---|---|
          | 8 | 델빌·판짱·조로아·리오르 — 구별되는 종, 구체적인 이유 |
          | 8 | 크로뱃·야도킹·노고치 — 이상하지만 **서로 다름** |
          | **12** | 따라큐·고디탱·아켄 — 귀여운 쪽 드리프트, 문장 반복 시작 |
          | **12** | **이브이·푸린·먹고자·잠만보·피카츄·토게피·이상해씨·파이리·꼬부기·뮤** — 전부 스타팅/마스코트, **한 문장을 12번 변형**, 피카츄를 "전설의 포켓몬"이라고 씀 |
          - 12개를 요구하면 모델이 **구별되는 12개를 못 찾아서 판정을 그만두고 유명한 걸 나열**한다. 그리고 **리스트 전체를 먼저 계획하므로 1등까지 같이 망가진다** — 버려지는 꼬리에만 국한되지 않는다.
        - **⚠️ 이유 문장 중복 가드 (`MAX_REASON_OVERLAP` 0.45)**: 위 붕괴의 조짐은 **종보다 문장에 먼저** 나타난다. 앞선 픽의 이유와 겹치는 픽은 실루엣 중복과 같은 방식으로 강등된다. **임계값은 고른 게 아니라 잰 것** — 정상 스캔의 회차 내 최악 쌍이 0.138·0.258(한국어 bigram Dice)인데 붕괴 회차는 중앙값 0.423·최악 0.842라 0.45가 그 사이다. 경계 사례("어두운 머리와 위를 향한 눈매" vs "검은 머리와 날카로운 눈매" = 0.571)는 **오탐이 아니라 정탐**이다 — 같은 관찰을 두 번 쓴 것이고 규칙 5가 이미 금지한다.
          - ⚠️ **이 가드가 붕괴를 되살리진 못한다**: 붕괴 회차를 재생해도 여전히 마스코트 5마리가 뜬다(전부 풀 멤버라 강등된 것이 다시 채워짐). 붕괴를 막는 건 **요청 개수의 몫**이고, 이 가드는 **보이게 만들고 한 문장이 다섯 번 인쇄되는 걸 막는** 역할이다.
          - ⚠️ 테스트 픽스처는 슬러그마다 **다른 이유**를 쓴다 — 공유 문자열을 쓰면 실루엣 상한 테스트가 조용히 이 가드를 테스트하게 된다.
        - **⚠️ `EXTRA_FAMOUS`는 `?debug`의 "POOL이 거부한 이름"으로 유지보수한다**: 추측하지 말고 **모델이 실제로 요청한 걸 보고 추가**할 것. 이 방식으로 에이팜·테르나·에몽가·쌈딱(+진화형)을 넣었다.
        - 실제 두 스캔을 새 순서로 재생한 결과: A = 델빌·판짱·조로아·리오르·**에이팜**(전부 모델 픽, 예전엔 4장 + 빈칸), B = **조로아·앱솔·식스테일** + 친숙한 로컬 2개(예전엔 크로뱃이 1등).
      - `buildCuratedPool`은 남겨뒀다 — `EXTRA_FAMOUS`가 "우리가 유명하다고 보는 후대 종" 문서 역할을 하고, 테스트가 금지 목록을 이걸로 검사하고, **위의 선호 정렬 기준이기도 하다.** 다만 **판정 게이트로 다시 쓰지 말 것.**
      - `?debug`에 **`POOL이 거부한 이름:`** 줄이 있다. 결과가 5개보다 적으면 **이 줄부터 볼 것** — 길면 게이트 문제, 비어 있는데도 짧으면 모델이 적게 답한 것이고 처방이 정반대다.
      - **⚠️ `MODEL RAW PICKS`도 이제 실제로 출력된다** — 이전 세션에서 수집만 하고 화면에 안 넣어서, 모델이 불안정한 건지 `pickGuards`가 뒤섞은 건지 구분할 방법이 계속 없었다.
      - **⚠️ 판정 결과가 5개보다 적으면 로컬 랭킹으로 채운다**(`usePokematchScan`). 모델 픽이 **앞에 순서까지 그대로** 오고 뒷자리만 채운다. 5마리 기준 화면에 1장만 뜨면 단호한 판정이 아니라 고장으로 보인다.
      - **⚠️ 후보 순서 섞기(`orderCandidatesForJudge`)는 이제 무의미하다** — 번호 매긴 목록을 프롬프트에 넣던 시절의 위치 편향 대책이었는데 **그 목록이 없어졌다.** 지우진 않았지만(구형 숫자 응답 경로가 씀), `?debug`의 `후보#`는 이제 **편향 지표가 아니라 단순 조회 번호**다.
    ...
    - **⚠️⚠️ `judge_unusable`는 토큰 한도가 아니다 (2026-08-16)**: 한도 초과는 **429/`rate_limited_upstream`** 로 따로 나온다. `judge_unusable`는 **HTTP 200이 왔는데 `normalizePicks`가 0개를 냈다**는 뜻이고, 원인이 두 가지인데 **처방이 정반대**다.
      1. **응답이 비어 있다** — `max_completion_tokens`는 **추론 + 답변을 합쳐서** 덮는데, 추론이 예산을 다 쓰면 답을 쓸 자리가 안 남는다. → 예산을 올린다. **실측으로 확인됨**: `finish=length completion=1400`에 내용이 닫히지 않은 `<think>` 블록뿐이었다. 700 → 1400 → **4000**. 프롬프트가 **2,307**(실측)이라 8K 한도 안에 5,693이 남는다.
      2. **이름이 안 풀린다** — 후보 목록을 프롬프트에서 뺀 뒤로 모델이 자유롭게 이름을 쓰는데, 그게 291종 풀 밖이면 버려진다. → 예산을 올려봐야 소용없고 풀/프롬프트를 봐야 한다.
      - `describeUnusable`가 어느 쪽인지 `?debug`에 찍는다: **`empty_content`·`thinking_never_finished`**(1번) / **`unresolved:이름,이름…`**(2번, 모델이 뭘 고르려 했는지까지 보인다) / `unparsed:…`(산문으로 답함) / `no_picks_in_json`. `finish=length`와 `prompt=`·`completion=` 숫자도 같이 나온다. **추측하지 말고 이 줄을 읽을 것.**
      - **⚠️⚠️⚠️ `reasoning_effort`를 생략하는 건 끄는 게 아니다 (2026-08-16 실측)**: Qwen 3.6은 **하이브리드 추론 모델이라 기본이 추론 ON**이다. 필드를 아예 안 보낸 요청도 `<think>`를 열고 예산 500을 전부 썼다(`thinking_never_finished finish=length completion=500`). 끄려면 **`reasoning_effort: "none"`을 명시**해야 한다. 400이 나면 필드를 빼는 폴백이 있는데 **그건 "끄기"가 아니라 "파라미터 미지원 모델용"** 이다 — 혼동하지 말 것.
      - **⚠️⚠️ 추론은 끝나지 않는다 — 3회 실측 (2026-08-16)**: 예산 **700 → 1400 → 4000** 전부 `finish=length`에 `<think>`가 안 닫힌 채 끝났다. **답을 한 글자도 안 썼다.** 8K 분당 한도에서 프롬프트 2,308을 빼면 최대 5,692이므로 **더 줄 여지가 거의 없다.** 그래서 순서를 뒤집었다 — **1차 = `direct`(추론 `none`, 1400토큰)**, **2차 = `think`(추론 `default`, 5600토큰)**. 이 엔드포인트의 일은 결과를 돌려주는 것이다. **`think`가 5600에서도 `finish=length`면 이 작업에서 추론은 종료하지 않는 것이니 예산을 또 올리지 말고 플랜을 지울 것.**
      - **⚠️ 재시도는 첫 시도와 "다른 설정"이어야 한다**: 예전엔 두 키가 **완전히 같은 요청**을 보내서, 요청 자체가 문제일 때 두 번째 키가 첫 번째의 실패를 **글자 그대로 재현**했다. 키가 하나뿐이어도 두 번째 플랜은 시도한다. `?debug`의 `provider`에 `primary/direct`·`secondary/think`처럼 **어느 키가 어느 방식으로** 답했는지 찍힌다.
      - **⚠️ `direct`의 출력 예산은 1400이다 (300 아님)**: 8개 픽 × 한국어 45자 이유 ≈ **1,000토큰**(한글 2.7토큰/자)이다. 예전 300은 **이유 문장을 조용히 잘라내고 있었다.**
      - **⚠️ 후보 목록 제거 효과도 추정이 틀렸다**: "약 1,050토큰 절약"이라고 적었는데 실측은 6,074 → **2,307**, 즉 **3,767 절약**이었다. 이 기능에서 글자수 기반 추정이 맞은 적이 한 번도 없다.
      - `extractJson`은 **닫히지 않은 `<think>`** 도 버린다 — 예산이 추론 중간에 끊기면 닫는 태그가 없어서, 남은 추론 산문에서 중괄호를 주워 엉뚱한 조각이 파싱될 수 있다.
