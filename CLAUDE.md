...
- **VL-6** PokéMatch("닮은 포켓몬 찾기") — 앱 완성, 자산 커밋됨, 브라우저 end-to-end 검증됨(실기기 최종확인만 남음):
  - **⚠️⚠️ 최종 판정자는 비전 LLM이다 (2026-08-09, 사용자 요청 "포켓매치가 좀 별로다" → "이미지로 분석하는 게 아니지?")**: 아래의 z-score 랭킹 엔진은 이제 **폴백**이고, 실제로 보이는 결과는 **Groq의 `qwen/qwen3.6-27b`(비전)** 가 **잘라낸 얼굴 사진을 직접 보고** 고른다.
    ...
    - **⚠️⚠️ 판정에 보내는 건 291종 전체가 아니라 "유사도 상위 40종"이다 (2026-08-15, 사용자 지적 "비슷한 거에서 빼와야 하는 거 아니야?")**: `matcher.ts`의 `shortlistFromPool(embedding, gallery, pokedex, 큐레이트풀, 40)`.
      - **문제**: 291종을 통째로 넘기면 판정이 **측정 가능한 어떤 것에도 근거하지 않는다**. 어떤 얼굴에든 수십 종이 그럴듯하게 맞고 모델은 그중 강한 선호가 없어서, **같은 사람이 연속 스캔할 때마다 완전히 다른 5마리**가 나왔다(사진이 조금만 달라져도 다른 종이 이긴다). 임베딩은 이 파이프라인에서 **개인을 실제로 측정하는 유일한 부분**인데, 그걸 판정이 아니라 표시용 %에만 쓰고 있었다.
      - **⚠️ 정렬은 `z`가 아니라 하이브리드 `score`로 한다**: 디버그에 찍히는 `z`는 raw z라 **허브(jigglypuff·mew·persian)가 모두에게 상위**를 차지한다. `score`는 60% unique-deviation z(= humanMean 차감)라 개인 고유 종이 떠오른다. 여기에 `HUB_EXCLUDE_SLUGS`(matchTopK와 같은 목록)를 적용한다. **raw z로 자르면 허브만 남아서 더 나빠진다.**
      - 40종은 **일부러 넓게** 잡았다 — 이 얼굴이 전혀 근거 없는 종만 걷어내고, 그중 무엇을 고를지는 여전히 **사진을 보는 모델의 몫**이다. 부수 효과로 요청이 회당 약 3,500 → **2,650토큰**(8K TPM에 3회).
      - `?debug`의 후보 줄에 `score=`와 `z=`가 같이 찍히므로, **원하는 종이 상위 40에 들었는지**를 바로 확인할 수 있다(안 들었으면 풀이 아니라 **shortlist 크기나 점수 방식**의 문제다).
    ...
    - **⚠️⚠️ 토큰 예산 — 이미지가 압도적으로 크다 (2026-08-15, Groq 429 본문으로 실측)**: 예전에 여기 적혀 있던 "이미지 448² ≈ 256토큰 / 회당 3,000~3,600"은 **틀렸다**. Qwen-VL의 28px/토큰 공식으로 추정한 값이었는데, 실제 429 응답이 `requested=5616`을 반환했다. 역산하면:
      - **텍스트 전체 약 1,707토큰** (시스템 467 + 후보 291종 목록 약 1,040 + 얼굴 측정문 200). 재현: `scratch/`가 아니라 `judgeProtocol.buildUserPrompt`로 직접 만들어 세면 된다.
      - **이미지 448² ≈ 3,500토큰** — 텍스트 전부의 두 배다. **추정치가 10배 이상 빗나갔다.**
      - 그래서 `JUDGE_IMAGE_SIZE`를 **448 → 256**으로 내렸다(토큰은 패치 수 = **면적**에 비례하므로 약 1/3). 회당 약 3,100토큰이 되어 8K TPM에 2회가 들어간다.
      - **⚠️ 이 값을 다시 올리면 429가 상시 발생한다.** 사용자가 겪은 "포켓몬이 안 닮았다"의 실제 원인이 이것이었다 — 판정이 매번 429로 막혀 **로컬 z-score 폴백(허브 편향)** 결과만 보고 있었다. 바꾸기 전에 `?debug`의 `requested=` 숫자를 먼저 확인할 것.
      - Groq 무료 티어는 30 RPM / 1K RPD / **8K TPM** / 200K TPD. **걸리는 건 TPD가 아니라 TPM이다** — 요청 하나가 한도의 상당 부분을 먹어서, 하루 총량이 남아 있어도 분당 한도로 막힌다. 신규 계정으로 바꿔도 동일한 이유로 즉시 429가 났다.
      - `reasoning_effort: "none"`으로 추론 토큰(300~900)을 제거했다. 사진 보고 고르는 건 지각 작업이라 추론의 값어치가 낮았다. `max_completion_tokens`는 300(예약분도 TPM에 계산된다).
      - 예산이 더 빠듯해지면 절삭 순서: ① **이미지 크기**(압도적 1순위) ② 얼굴 측정문(200) ③ 풀 크기 ④ `reason`(**효과 미미하니 마지막**).
