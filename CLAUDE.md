...
- **VL-7** VrmMotion("VRM Capture", `/vrmmotion`) — 실시간 3D 모션캡쳐 앱, 2026-08-02~06 구현:
  ...
  - **FBX / glTF 업로드 지원 (2026-08-06)**: `.vrm` 외에 **`.fbx`·`.glb`·`.gltf`** 캐릭터도 업로드해 쓸 수 있다. 트래커는 three-vrm의 **정규화 휴머노이드**만 구동하므로, 非VRM 리그는 `lib/vrm/humanoidRigger.ts`가 VRM 휴머노이드로 **어댑트**한다(`VRMHumanoid`는 뼈 맵만 주면 직접 생성 가능하다는 점을 이용). 추상화는 `lib/vrm/motionAvatar.ts`의 `MotionAvatar`(브리지·훅·캔버스가 이제 `VRM` 대신 이 타입을 받는다), 로더 분기는 `vrmScene.ts`의 `loadAvatar(source, nameHint)`.
    ...
    4. **기본 제공 아바타 프리셋 (2026-08-06)**: `lib/vrm/avatarPresets.ts`의 `AVATAR_PRESETS`가 UI 선택 목록(기본 VRM / 스파이더맨 FBX). 앱 추가처럼 프리셋 추가도 항목 1개면 된다. FBX 프리셋은 **텍스처 테이블**이 필요하다 — 원본 FBX에 **텍스처 참조가 아예 없어서**(바이너리 스캔·로더 요청 URL 0개로 확인) 머테리얼↔맵 연결이 데이터가 아니라 규약이다. Suit는 머테리얼 이름이 `"1".."5"` 로 `Suit_*.1001..1005` 와 일치(빨강/파랑 타일 분포도 일치)해 근거가 확실하고, 액세서리는 추정(밝고 디테일한 유일한 타일 `Acc_*.1011`을 `Lense`에, 나머지는 순서대로). `Webs` 머테리얼은 원본에 타일이 없어 미적용. **틀린 부분이 보이면 이 표의 타일만 바꾸면 된다.** `scratch/check_preset_textures.mjs`가 실제 FBX 머테리얼 이름·파일 존재를 교차검증(현재 11개 중 `Webs` 제외 10개 매칭, 누락 파일 0).
    5. **자산 파이프라인 (⚠️ 원본은 커밋 금지)**: Sketchfab류 원본은 4096×4096 PNG 세트로 **636MB**(노멀맵 1장이 57MB)라 git·Vercel·브라우저 모두 불가능. 그래서 원본 `public/models/spider-man-brand-new-day/`는 **gitignore**하고, `node scratch/build_avatar_assets.mjs`가 웹용으로 변환한 `public/models/spiderman/`만 커밋한다(FBX 30MB 그대로 + 텍스처 21장을 1024/512 WebP로 → **635MB → 1.6MB**). Metallic/Roughness는 FBXLoader가 만드는 MeshPhong에 슬롯이 없어 제외. 원본을 교체하면 이 스크립트를 다시 돌릴 것.
    6. **검증**: `tests/humanoidRigger.test.ts` — 이름 매핑, 스케일/접지, A-pose→T-pose 보정, 정면 180° 보정, 그리고 **"어댑트된 A-pose 리그가 T-pose 리그와 동일하게 반응한다"** 는 end-to-end 등가성(정규화 본에 같은 회전을 넣고 raw 손 위치 델타 비교). 보정을 끄면 같은 입력에서 손 위치가 **0.226m** 어긋나므로(임계 0.05m) 이 테스트는 실제로 회귀를 잡는다.
    7. **실제 FBX 검증 (스파이더맨 모델)**: `scratch/inspect_avatar_rig.mjs`로 실물 확인 — Mixamo 리그(`mixamorigHips` 식, 콜론 없이 붙은 이름도 토크나이저가 처리)로 필수 본 15개 전부 매핑, 967→1.6m 스케일·정면 180°·팔 A-pose 52° 보정 후 rest dir이 정확히 `(∓1,0,0)`, side-raise가 손을 실제로 올림(dy +0.25). ⚠️ 이 모델은 **스킨드 메시 7개가 각자 52본 스켈레톤 복사본**을 갖는데, 복사본들이 **0 오프셋으로 중첩**돼 있고 최상단이 나머지 전체의 조상이라 `collectBones`의 **최소 depth 선택**이 곧 전체 메시를 구동한다(이 선택 규칙을 바꾸면 일부 메시만 움직이게 된다).
