...
> 런처. 브라우저 탭 타이틀은 **"VisionLab"**(`app/layout.tsx` metadata). 홈 히어로 H1은 아직 "VisionLab AI"로 표기됨
> (원하면 통일 가능). 개발: `npm run dev`(3000 사용 중이면 다른 포트), `npm run build`, `npm test`.
...
- (선택) 홈 히어로 H1 "VisionLab AI" → "VisionLab" 통일 여부.
