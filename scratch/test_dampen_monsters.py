...
EXCLUDE_SLUGS = set([
    ...
    "chi_yu", "goldeen", "seaking", "koraidon", "mankey", "primeape", "annihilape", "sandslash", "sandshrew"
])
...
def main():
    ...
    print("=== DAMPENED LOW-BASELINE OUTLIERS MATCHES ===")
    for s in subjs:
        ...
        for i, slug in enumerate(species):
            ...
            if shape in NON_HUMAN_EXCLUDE_SHAPES or slug in EXCLUDE_SLUGS:
                continue
            if mu_raw[i] < 0.25: # Dampen low baseline monster outliers
                continue
        ...
        top5_slugs = [f"{x[1]} ({pokedex.get(x[1], {}).get('nameKo') or x[1]})" for x in scored[:5]]
        print(f"{s}: {', '.join(top5_slugs)}")
